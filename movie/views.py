...
def index(request):
	...
	if query:
		url = 'https://www.omdbapi.com/?apikey=97824716&s=' + query
...
def pagination(request, query, page_number):
	url = 'https://www.omdbapi.com/?apikey=97824716&s=' + query + '&page=' + str(page_number)
	response = requests.get(url)
	movie_data = response.json()
...
def movieDetails(request, imdb_id):
	...
	if Movie.objects.filter(imdbID=imdb_id).exists():
		...
		url = 'https://www.omdbapi.com/?apikey=97824716&i=' + imdb_id
		response = requests.get(url)
		movie_data = response.json()
